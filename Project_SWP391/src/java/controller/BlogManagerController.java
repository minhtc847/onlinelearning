...
public class BlogManagerController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet BlogManagerController</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet BlogManagerController at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        int active = Integer.parseInt(request.getParameter("active") == null ? "-1" : request.getParameter("active"));
        ...
        if (active != -1) {
            if (listPost != null) {
                for (posts post : listPost) {
                    if (post.getIs_feature() == active) {
                        listPostActive.add(post);
                    }
                }
            }
        }else{
            listPostActive = listPost;
        }
